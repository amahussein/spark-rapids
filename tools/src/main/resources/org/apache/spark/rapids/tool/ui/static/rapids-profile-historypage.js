...
$(document).ready(function() {
  ...
  var historySummary = $("#profile-history-summary");
  ...
  $.getJSON(uiRoot + "/api/v1/applications", appParams, function(response, _ignored_status, _ignored_jqXHR) {
    ...
    for (var i in response) {
      ...
      for (var j in app["attempts"]) {
        ...
        attempt["compareUrl"] =(currentAppID != id ? uiRoot + "/history/" + currentAppID + "/compare/results?id=" +
            id : "");
        array.push(attempt);
      }
    ...
    $.get(uiRoot + "/static/rapids/rapids-profile-historypage-template.html", function(template) {
      var sibling = historySummary.prev();
      historySummary.detach();
      var apps = $(Mustache.render($(template).filter("#rapids-profile-history-summary-template").html(),data));
