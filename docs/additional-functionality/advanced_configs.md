...
<a name="sql.cubf.diagnosticMetrics.enabled"></a>spark.rapids.sql.cubf.diagnosticMetrics.enabled|When enabled, wires optional cuBF build/probe diagnostic accumulators for planner-emitted cuBF markers. When disabled, or when no cuBF bfId is present, no cuBF diagnostic accumulators are registered.|false|Runtime
